...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(Math.max(-1, Math.min(1, csin)));
}
...
function isPowerOfTwo(num) {
  ...
  const lg2 = Math.log(num) / Math.log(2);
...
function getSumOfNumbers(...numbers) {
  const factor = Math.max(
    ...numbers.map((num) => {
      const posPoint = num.toString().indexOf('.');

      return posPoint > -1 ? 10 ** posPoint : 1;
    })
  );

  return numbers.reduce((acc, num) => acc + num * factor, 0) / factor;
}
